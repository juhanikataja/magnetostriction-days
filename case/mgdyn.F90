    ...
    DO t=1,IP % n
       ...
       !ELSE ! DEBUG
         IF(ExternalHB) THEN
           ...
           if (firsttime_hb .and. abs(mu-dhdb(1,1))/mu>0.01_dp) THEN ! DEBUG
             print *, 'dhdb(1) = ', dhdb(1,1)! DEBUG
             print *, 'mu = ', mu ! DEBUG
             print *, '|sloc| = ', sqrt(sum(msmodel % sloc*msmodel % sloc))
             firsttime_hb = .false. ! DEBUG
           END IF
           mu = dHdB(1,1)
         END IF
       !END IF ! DEBUG
