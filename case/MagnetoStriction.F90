...
MODULE MagnetoStriction
  ...
  TYPE MSModel_t
    ...
    REAL(KIND=dp) :: PoissonRatio, YoungModulus, Density
    INTEGER :: s_n=0, av_nd(2)=0, av_np=0, av_n=0, av_edgebasisdegree=0, s_nd(2)=0
    LOGICAL :: av_piola 
    REAL(KIND=dp), ALLOCATABLE :: rotwbasis(:,:), wbasis(:,:), aloc(:), sloc(:,:)
...
!-------------------------------------------------------------------------------
  SUBROUTINE CollectMSModel(Material, uelement, MSModel, Model)
!-------------------------------------------------------------------------------
    IMPLICIT NONE
    TYPE(ValueList_t), INTENT(IN), POINTER :: Material
    ...
    CONTAINS
      ...
      SUBROUTINE AllocateLocalSolutionArrays()
        ...
        IF (av_nd > MSModel % av_nd(2)) THEN
          ALLOCATE(MSModel % RotWBasis(av_nd,3),&
